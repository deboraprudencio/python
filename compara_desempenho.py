...
class Runtime:
    '''
    Recebe uma lista de métodos, uma lista de inputs e um método de teste (opcional)
    runtime() retorna o tempo de execução deum método para um parâmetro da lista
    run_all() compara o tempo de execução dos métodos da lista pra cada input
    Retorna qual foi mais rápido pra cada parâmetro e qual foi mais rápido no geral
    Obs: só funciona para métodos e inputs com tempo de execuçção significativo
    '''

    def __init__(self, method_list, input_list, tester_method = None):
